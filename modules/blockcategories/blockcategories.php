...
class BlockCategories extends Module
{
	...
	{	
		...
		if($banderaMarca== 1)
		{
			...
			{	
				$id_category= $resultId["id_category"];		
	...
	public function hookLeftColumn($params)
	{
		...
		if (!$this->isCached('blockcategories.tpl', $cacheId))
		{
			...
			/*Área modificada Start*/
			//Result filtrado por marca o search_query Start
			$id_marca= Tools::getValue('id_manufacturer');
			
			if(isset($_GET['search_query']))
			{
				$searchQueryClean = preg_replace('([^A-Za-z0-9 ])', '', $_GET['search_query']);
				$searchQueryClean = strtoupper($searchQueryClean);
				$resultManufacturer = Db::getInstance()->executeS('SELECT `id_manufacturer` FROM `'._DB_PREFIX_.'manufacturer` m WHERE m.`name` ='.'"'.$searchQueryClean.'"');

				/*Si es una palabra cetegoría*/
				if(empty($resultManufacturer))
				{
					$resultCategory = Db::getInstance()->executeS('SELECT cl.`id_category` FROM `'._DB_PREFIX_.'category_lang` cl INNER JOIN `'._DB_PREFIX_.'category` c ON (cl.`id_category` = c.`id_category`) WHERE `name` ='.'"'.$searchQueryClean.'" AND  `active` =1');
				}
			}
			...
			if($id_marca!= false)
        	{
				...
				INNER JOIN `'._DB_PREFIX_.'product` pd ON (pd.`id_category_default` = c.`id_category`)
				WHERE (c.`active` = 1 OR c.`id_category` = '.(int)Configuration::get('PS_HOME_CATEGORY').')
				AND pd.`id_manufacturer` ='.$id_marca.'
				...
				//Bandera para indicar que debe buscar por marca en el método getTree
				$banderaMarca= 1;
			}

			elseif (!empty($resultManufacturer)) 
			{
				$id_marca_search= $resultManufacturer[0]["id_manufacturer"];
				$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT c.id_parent, c.id_category, cl.name, cl.description, cl.link_rewrite
				FROM `'._DB_PREFIX_.'category` c
				INNER JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category` AND cl.`id_lang` = '.(int)$this->context->language->id.Shop::addSqlRestrictionOnLang('cl').')
				...
				INNER JOIN `'._DB_PREFIX_.'product` pd ON (pd.`id_category_default` = c.`id_category`)
				WHERE (c.`active` = 1 OR c.`id_category` = '.(int)Configuration::get('PS_HOME_CATEGORY').')
				AND pd.`id_manufacturer` ='.$id_marca_search.'
				AND c.`id_category` != '.(int)Configuration::get('PS_ROOT_CATEGORY').'
				'.((int)$maxdepth != 0 ? ' AND `level_depth` <= '.(int)$maxdepth : '').'
				'.$range.'
				AND c.id_category IN (
				...
				//Bandera para indicar que debe buscar por search_query en el método getTree
				$banderaMarca= 1;
			}
			//Result filtrado por marca o search_query Start
			...
			else
			{
				...
				{
					$id_categoria_search= $resultCategory[0]["id_category"];
					$resultCategoryChildren = Db::getInstance()->executeS('SELECT `id_category` FROM `'._DB_PREFIX_.'category` WHERE `id_parent` ='.$id_categoria_search.' AND `active` =1');
					
					if(empty($resultCategoryChildren))
						$categoryChildren= 'pd.id_category_default="'.$id_categoria_search.'" AND ';

					else{
						$categoryChildrenArray= array();
						$categoryChildren= "";
						foreach ($resultCategoryChildren as $i => $resultCategoryChild) {
							if($i== sizeof($resultCategoryChildren)-1) 
								$categoryChildrenArray[$i]= 'pd.id_category_default="'.$resultCategoryChild["id_category"].'" AND ';

				            else
				            {
				                if($i== 0)
				                   $categoryChildrenArray[$i]= 'pd.id_category_default="'.$resultCategoryChild["id_category"].'" OR ';
				                
				                else
				                   $categoryChildrenArray[$i]= 'pd.id_category_default="'.$resultCategoryChild["id_category"].'" OR ';
				            }
						}
						$categoryChildren = implode("", $categoryChildrenArray);
						$categoryChildren= 'pd.id_category_default="'.$id_categoria_search.'" OR '.$categoryChildren;
					}
					...
					INNER JOIN `'._DB_PREFIX_.'product` pd ON (pd.`id_category_default` = c.`id_category`)
					WHERE (c.`active` = 1 OR c.`id_category` = '.(int)Configuration::get('PS_HOME_CATEGORY').')
					AND '.$categoryChildren.' c.`id_category` != '.(int)Configuration::get('PS_ROOT_CATEGORY').'
					'.((int)$maxdepth != 0 ? ' AND `level_depth` <= '.(int)$maxdepth : '').'
					'.$range.'
					AND c.id_category IN (
						SELECT id_category
					...
					$this->smarty->assign('palabraCategoria', 'pcategoria');
				}
				/*Área modificada End*/
			...
			/*Área modificada Start*/
			$blockCategTree = $this->getTree($resultParents, $resultIds, $maxdepth, ($category ? $category->id : null), null, $banderaMarca);

			//Condición para mostrar las categorías de una marca en específico o search_query
			$id_marca= Tools::getValue('id_manufacturer');
			if($id_marca!= false || $id_marca_search!= "")
			{
				if($id_marca!= false)
				{
					$manufacturerObject = new Manufacturer((int)$id_marca, $this->context->language->id);
					$i= 0;
					foreach ($blockCategTree["children"] as $hijo) {	
						$lowerName= strtolower($manufacturerObject->name);
						$ligaPrincipal= $this->context->link->getPageLink('index',true);
						$ligaConcatenada= $ligaPrincipal.$id_marca."_".$lowerName."?categoria=".$hijo["id"];	
						$blockCategTree["children"][$i]["link"]= $ligaConcatenada;
						$i++;
					}
				}
				...
				{
					$manufacturerObject = new Manufacturer((int)$id_marca_search, $this->context->language->id);
				
					$i= 0;
					foreach ($blockCategTree["children"] as $hijo) {	
						$lowerName= strtolower($manufacturerObject->name);
						$ligaPrincipal= $this->context->link->getPageLink('index',true);
						$ligaConcatenada= $ligaPrincipal.$id_marca_search."_".$lowerName."?categoria=".$hijo["id"];	
						$blockCategTree["children"][$i]["link"]= $ligaConcatenada;
						$i++;
					}
				}
