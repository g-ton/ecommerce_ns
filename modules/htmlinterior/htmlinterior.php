...
class HTMLINTERIOR extends Module
{
	public function __construct()
	{
		$this->name = 'htmlinterior';
		...
		$this->monolanguage_content = 'Htmlinterior_MONOLANGUAGE';
		$this->text_editor_content = 'Htmlinterior_TEXTEDITOR';
		$this->content_wrapper = 'Htmlinterior_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Htmlinterior_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Htmlinterior_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('HTMLInterior');
		$this->description = $this->l('Inferior');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !HTMLINTERIORModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = HTMLINTERIORModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = HTMLINTERIORModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					HTMLINTERIORModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					HTMLINTERIORModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																HTMLINTERIORModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class HTMLINTERIORModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( HTMLINTERIORModel::createFilesTable()
				&& HTMLINTERIORModel::createContentTable());
	}
