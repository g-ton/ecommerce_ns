...
class HTMLLATERAL extends Module
{
	public function __construct()
	{
		$this->name = 'htmllateral';
		$this->description = 'Lateral';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Htmllateral_MONOLANGUAGE';
		$this->text_editor_content = 'Htmllateral_TEXTEDITOR';
		$this->content_wrapper = 'Htmllateral_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Htmllateral_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Htmllateral_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('HTMLLateral');
		$this->description = $this->l('Lateral');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !HTMLLATERALModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = HTMLLATERALModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = HTMLLATERALModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					HTMLLATERALModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					HTMLLATERALModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																HTMLLATERALModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class HTMLLATERALModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( HTMLLATERALModel::createFilesTable()
				&& HTMLLATERALModel::createContentTable());
	}
