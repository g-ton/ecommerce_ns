...
class ProductsCategory extends Module
{
	...
	public function hookProductFooter($params)
	{
		...
		if (!$this->isCached('productscategory.tpl', $this->getCacheId($cache_id)))
		{
			...
			// Get infos
			//$category_products = $category->getProducts($this->context->language->id, 1, 100); /* 100 products max. Originalmente*/
			$category_products = $category->getProducts($this->context->language->id, 1, 6, null, null, null, null, true, 6); /* 6 products max. */
	...
	public function hookDisplayHomeConjunto($params)
	{
		...
		if (!$this->isCached('productscategory.tpl', $this->getCacheId($cache_id)))
		{
			...
			// Get infos (getProductsConjunto nuevo método para obtener productos comprados en conjunto)
			$category_products = $category->getProductsConjunto($this->context->language->id, 1, 6, null, null, null, null, true, 32, null, null, $reference); /* 32 products max. */

			/*Modificado para categorizar productos comprados en conjunto - Start*/
			if(!empty($category_products))
			{
				$categoriasArray= array();
				$categoriasDistinct = array(); 
				$categoriasFinal = array(); 
				foreach ($category_products as $key => $category_product)
					$categoriasArray[$key]= $category_product["category_default"];

			    foreach($categoriasArray as $key=>$val)    
			        $categoriasDistinct[$val] = true; 

	    		$categoriasDistinct = array_keys($categoriasDistinct); 
	    		$tamCategorias= sizeof($categoriasDistinct);

	    		if($tamCategorias>= 4)
	    		{
		    		for ($i=0; $i <4 ; $i++) 
		    			$categoriasFinal[$i]= $categoriasDistinct[$i];
		    		
		    		$categoriasDistinct= $categoriasFinal;
	    		}
    		}
			/*Modificado para categorizar productos comprados en conjunto - End*/
		...
		return $this->display(__FILE__, 'productscategoryConjunto.tpl', $this->getCacheId($cache_id));
	...
	public function hookHeader($params)
	{
		...
		$this->context->controller->addJS($this->_path.'js/productscategory.min.js');
