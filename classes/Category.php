...
class CategoryCore extends ObjectModel
{
    ...
    //getProductsConjunto nuevo método para obtener productos comprados en conjunto
    public function getProductsConjunto($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null, $id_product)
    {
        $sql = 'SELECT id_producto_relacionado FROM `'._DB_PREFIX_.'productos_conjunto` WHERE `id_producto` = "'.$id_product.'"';
        $resultConjunto = Db::getInstance()->executeS($sql);

        if(empty($resultConjunto))
        {
            return array();
        }
        ...
        $customWhere= array();
        $resultWhere= "";
        foreach ($resultConjunto as $iterador => $productoRelacionado) 
        {
            if($iterador== sizeof($resultConjunto)-1) 
            {
                $customWhere[$iterador]= 'p.reference="'.$productoRelacionado["id_producto_relacionado"].'" AND ';
            }

            else
            {
                if($iterador== 0)
                    $customWhere[$iterador]= 'p.reference="'.$productoRelacionado["id_producto_relacionado"].'" OR ';
                
                else
                    $customWhere[$iterador]= 'p.reference="'.$productoRelacionado["id_producto_relacionado"].'" OR ';
            }
        }
        $resultWhere = implode("", $customWhere);      
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity'.(Combination::isFeatureActive() ? ', IFNULL(product_attribute_shop.id_product_attribute, 0) AS id_product_attribute,
                ...
                WHERE '.$resultWhere.' product_shop.`id_shop` = '.(int)$context->shop->id
                    .($active ? ' AND product_shop.`active` = 1' : '')
                    .($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '')
                    .($id_supplier ? ' AND p.id_supplier = '.(int)$id_supplier : '');
