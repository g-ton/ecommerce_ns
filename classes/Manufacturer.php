...
class ManufacturerCore extends ObjectModel
{
    ...
    public static function getProducts($id_manufacturer, $id_lang, $p, $n, $order_by = null, $order_way = null,
        $get_total = false, $active = true, $active_category = true, Context $context = null, $categoriaGet= null)
    {
        if (!$context) {
        ...
        if ($get_total) {
            ...
            //Validación para saber si viene la categoría definida o no
            ...
            else
            {
                $sql = '
                    ...
                    FROM `'._DB_PREFIX_.'product` p
                    '.Shop::addSqlAssociation('product', 'p').'
                    WHERE p.`id_category_default` = '.$categoriaGet.' AND p.id_manufacturer = '.(int)$id_manufacturer
                    .($active ? ' AND product_shop.`active` = 1' : '').'
                    '.($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').'
                    AND EXISTS (
                        SELECT 1
